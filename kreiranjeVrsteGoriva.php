...
$nazivVrste = filter_input(INPUT_POST, 'nazivVrste');
$dobavljac = filter_input(INPUT_POST, 'dobavljac');


if ($_SERVER["REQUEST_METHOD"] == "POST") {
    if (empty($nazivVrste) || empty($dobavljac)) {
        $ispisGreski .= "Niste popunili polja!<br>";
    }
    else {
        $upit = "INSERT INTO vrsta_goriva (id_vrste_goriva,naziv_vrste_goriva,dobavljac) VALUES (default, '$nazivVrste', '$dobavljac');";
        unosUpdate($upit);
        $ispisGreski .= "Uspješno ste kreirali novu vrstu goriva!<br>";
    }
    $korisnik=$_SESSION['idKorisnika'];
    $zapis="Kreirana je vrsta goriva";
...
<head>
    <title>Kreiranje vrste goriva</title>
    <meta charset="utf-8">
...
<body>
    ...
    <section class="centar">
        <div id="sadrzaj">
            <h1>Kreiranje vrste goriva</h1>
            <form id="formaKreiranjeVrsteGoriva" method="post">
                <div id="sadrzajForme">
                    <h2>Obrazac za kreiranje vrste goriva</h2>

                    <label for="nazivVrste">Naziv goriva:</label>
                    <input type="text" id="nazivVrste" name="nazivVrste" placeholder="Naziv" autofocus="autofocus">
                    <br>

                    <label for="dobavljac">Dobavljač:</label>
                    <input type="text" id="dobavljac" name="dobavljac" placeholder="Dobavljač">
                    <br>

                    <br>
                    <input type="submit" value="Kreiraj vrstu goriva">
                    <br>
